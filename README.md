<p align="center">
  <img src="https://img.shields.io/badge/Jetpack%20Compose-1.7+-007FFF?style=for-the-badge&logo=android&logoColor=white&labelColor=000000" alt="Jetpack Compose" />
  <img src="https://img.shields.io/badge/Material%203-Dynamic%20Colors-FF4081?style=for-the-badge&logo=materialdesign&logoColor=white&labelColor=000000" alt="Material 3" />
  <img src="https://img.shields.io/badge/Pexels%20API-4K%20Photos%20%26%20Videos-26A641?style=for-the-badge&logo=pexels&logoColor=white&labelColor=000000" alt="Pexels API" />
  <img src="https://img.shields.io/badge/Live%20Video%20Wallpapers-Working-F57C00?style=for-the-badge&logo=android&logoColor=white&labelColor=000000" alt="Live Wallpapers" />
  <img src="https://img.shields.io/badge/Kotlin%20Coroutines%20%26%20Flow-1.8+-00BCD4?style=for-the-badge&logo=kotlin&logoColor=white&labelColor=000000" alt="Coroutines" />
</p>

<div align="center">
  <h1>WallpaperCompose 🎨✨</h1>
  <h3>
    <i>Next-level Android wallpaper app built 100% with Jetpack Compose</i><br>
    <strong>4K Photos • Live Video Wallpapers • Material You • Pexels API</strong>
  </h3>

  <br/><br/>

  <a href="#-features"><strong>Features</strong></a> •
  <a href="#-screenshots"><strong>Screenshots</strong></a> •
  <a href="#-demo-video"><strong>Demo</strong></a> •
  <a href="#-tech-stack"><strong>Tech Stack</strong></a> •

</div>

<br/>

## 🌟 Features at a Glance

<table>
<tr>
<td>

**Photos Experience**
- 4K curated & category photos from Pexels
- 20+ handpicked categories (Nature, Cyberpunk, Aurora, Rain, Galaxy…)
- Smooth category chips with animated selection
- Shimmer loading + crossfade transitions
- One-tap set: Home • Lock • Both (API 24+)

</td>
<td>

**Live Video Wallpapers**
- Download HD videos with real-time progress (%, MB)
- Animated download dialog + Linear/Circular indicators
- Auto-detect new videos via SharedPreferences listener
- Muted, looping playback with proper visibility handling
- System live wallpaper chooser integration

</td>
</tr>
</table>

**Also included**
- Material 3 dynamic theming (light/dark/system)
- Clean MVVM + StateFlow architecture
- Coil for fast & cached image loading
- Retrofit + OkHttp with logging
- Error states & empty states with beautiful UI

<br/>

## 🎬 Demo Video

![Demo](https://raw.githubusercontent.com/ADMusab12/android-compose-wallpaper/main/demo/wallpaper_demo.gif)

## 📸 Screenshots

<div align="center">
<table>
<tr>
<td><img src="demo/d1.jpg" width="220" alt=""/></td>
<td><img src="demo/d2.jpg" width="220" alt=""/></td>
<td><img src="demo/d3.jpg" width="220" alt=""/></td>
</tr>
<tr>
<td><img src="demo/d4.jpg" width="220" alt=""/></td>
<td><img src="demo/d5.jpg" width="220" alt=""/></td>
<td><img src="demo/d6.jpg" width="220" alt=""/></td>
</tr>
</table>
</div>

<br/>

- Free Pexels API Key → https://www.pexels.com/api/

---

⭐ Loved this project? Give it a star and support the work!



